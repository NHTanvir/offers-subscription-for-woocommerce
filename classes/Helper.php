...
class Helper {
	...
	public static function get_posts( $args = [], $show_heading = false, $show_cached = false ) {
		...
		// use cache
		if( true === $show_cached && ( $cached_posts = wp_cache_get( "osfw_{$_args['post_type']}", 'osfw' ) ) ) {
			$posts = $cached_posts;
		}
		...
		// don't use cache
			...
			wp_cache_add( "osfw_{$_args['post_type']}", $posts, 'osfw', 3600 );
		}

		$posts = $show_heading ? [ '' => sprintf( __( '- Choose a %s -', 'osfw' ), $_args['post_type'] ) ] + $posts : $posts;

		return apply_filters( 'osfw_get_posts', $posts, $_args );
	}
	 ...
	 *
	 * It'll look into /offers-subscription-for-woocommerce directory of your active theme
	 * first. if not found, default template will be used.
	 * can be overwriten with offers-subscription-for-woocommerce_template_overwrite_dir hook
	 *
	...
	public static function get_template( $slug, $base = 'views', $args = null ) {
		...
		$overwrite_template_dir = apply_filters( 'osfw_template_overwrite_dir', get_stylesheet_directory() . '/offers-subscription-for-woocommerce/', $slug, $base, $args );
		
		// default template directory
		$plugin_template_dir = dirname( Offers_Subscription_For_WooCommerce ) . "/{$base}/";
		...
		elseif ( file_exists( $plugin_template_path ) ) {
			...
			return __( 'Template not found!', 'offers-subscription-for-woocommerce' );
		}
	}
}